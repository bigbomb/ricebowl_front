<template>
  <el-row id="purchaseContractList">
    ...
    <!--列表-->
    <el-col>
      <el-table ref="purchaseTable" :data="purchaseContracts" highlight-current-row v-loading="listLoading" element-loading-text="拼命加载中" @selection-change="handleSelectionChange" style="width: 100%;">
        <el-table-column type="selection" width="55">
        </el-table-column>

        <el-table-column prop="id" label="id" sortable fixed="left" v-if="isshow" width="80">
        </el-table-column>

        <el-table-column prop="memberid" label="会员id" sortable fixed="left" width="150" v-if="isshow">
        </el-table-column>
        <el-table-column prop="purchasestatus" label="采购单状态" fixed="left" sortable width="150">
        </el-table-column>
        ...
        <el-table-column prop="contractno" label="销售合同号" sortable width="150" v-if="isshow">
        </el-table-column>
        <el-table-column prop="customername" label="客户名称" sortable width="250" v-if="isshow">
        </el-table-column>
        <el-table-column prop="purchaseweight" label="采购吨位" sortable width="150">
        </el-table-column>
        <el-table-column prop="purchaseamount" label="采购金额" sortable width="150">
        </el-table-column>
        <el-table-column prop="payment" label="付款方式" sortable width="150">
        </el-table-column>
        ...
        <el-table-column prop="verifyby" label="审核人" sortable width="100">
        </el-table-column>
        <el-table-column prop="operation" fixed="right" label="操作" width="100">
          <template slot-scope="scope">
              <el-popover trigger="hover" placement="bottom">
                ...
                <div><el-button type="text" size="small" @click='editShow(scope.row)' >明细</el-button></div>
                ...
                <div>
                  ...
                  <el-button type="text" v-if="scope.row.purchasestatus==='待审核'" size="small" @click='verify(scope.row)'>审核</el-button>
                </div>
                <div>
                ...
                <div slot="reference" class="name-wrapper">
                 可操作项<i class="el-icon-arrow-down el-icon--right"></i>
                </div>
              </el-popover>
          </template>
        </el-table-column>
    ...
    <el-col :span="2">
      <el-dialog :title="thistitle" :visible.sync="supplyDialogFormVisible" width="1250px">
        <div>
          <el-form :model="supplyerForm" status-icon :rules="rules" ref="supplyerForm" :inline="true" label-width="100px" class="demo-form-inline">
            <el-col :span="7">
              <el-form-item label="供应商名称" prop="supplyername">
                <el-autocomplete v-model="supplyerForm.supplyername" :fetch-suggestions="querySearchAsync" placeholder="请输入客户名称" @select="handleSelect" class="autoinputwidth">
                </el-autocomplete>
              </el-form-item>
            </el-col>
            <el-col :span="3">
            ...
            <el-col :span="7">
              <el-form-item label="采购日期" prop="purchasedate">
                <el-date-picker class="autoinputwidth" v-model="supplyerForm.purchasedate" type="date" placeholder="选择日期" ></el-date-picker>
              </el-form-item>
            </el-col>
            <el-col :span="7">
            ...
            <el-col :span="24">
              <el-form-item label="附加条款" prop="remark">
                <el-input :rows="3" class="inputwidth" type="textarea" placeholder="请输入备注" auto-complete="off" v-model="supplyerForm.remark"></el-input>
              </el-form-item>
            </el-col>
            <el-form-item label="商品明细">
              <el-button v-if="supplyerForm.purchasestatus==='待审核'||supplyerForm.purchasestatus===''" size="mini" @click="addRow()">+</el-button>
              <el-table style="width:1100px" :data="productGridData" max-height="500" :span-method="arraySpanMethod" show-summary :summary-method="getSummaries" class="el-tb-edit" ref="gridTable" highlight-current-row>
                ...
                <el-table-column property="productname" label="名称" width="200" >
                  <template slot-scope="scope">
                    ...
                    <el-autocomplete class="autoInputwidth" v-model="scope.row.productname" v-if="scope.row.status!=='在库'||scope.row.status==undefined" :fetch-suggestions="queryProductNameSearchAsync" placeholder="请输入商品名称">
                ...
                <el-table-column property="productspec" label="规格" width="200" >
                  <template slot-scope="scope">
                    <!-- <el-input size="mini" v-if="scope.row.status!=='在库'" v-model="scope.row.productspec" placeholder="请输入内容"></el-input> -->
                     <el-autocomplete class="autoInputwidth" v-model="scope.row.productspec" v-if="scope.row.status!=='在库'||scope.row.status==undefined" :fetch-suggestions="queryProductspecSearchAsync" placeholder="请输入商品规格">
                      <template slot-scope="{ item }">
                        <div class="name">{{ item.value }}</div>
                        <span hidden>{{ item.id }}</span>
                        <el-tooltip  content="删除后重新点击输入框刷新" placement="bottom" effect="light">
                ...
                <el-table-column property="productfactory" label="钢厂" width="150" >
                  <template slot-scope="scope">
                    ...
                    <el-autocomplete class="autoInputwidth" v-model="scope.row.productfactory" v-if="scope.row.status!=='在库'||scope.row.status==undefined" :fetch-suggestions="queryProductfactorySearchAsync" placeholder="请输入钢厂">
                      <template slot-scope="{ item }">
                        <div class="name">{{ item.value }}</div>
                        <span hidden>{{ item.id }}</span>
                        <el-tooltip  content="删除后重新点击输入框刷新" placement="bottom" effect="light">
                ...
                <el-table-column property="productmark" label="材质" width="150">
                  <template slot-scope="scope">
                    ...
                    <el-autocomplete class="autoInputwidth" v-model="scope.row.productmark" v-if="scope.row.status!=='在库'||scope.row.status==undefined" :fetch-suggestions="queryProductmarkSearchAsync" placeholder="请输入商品材质">
                      <template slot-scope="{ item }">
                        <div class="name">{{ item.value }}</div>
                        <span hidden>{{ item.id }}</span>
                         <el-tooltip  content="删除后重新点击输入框刷新" placement="bottom" effect="light">
                ...
                <el-table-column property="weight" label="重量(吨)" width="120" >
                  <template slot-scope="scope">
                    <el-input size="mini" v-if="scope.row.status!=='在库'" v-model="scope.row.weight" placeholder="请输入内容" ></el-input>
                    <span>{{scope.row.weight}}</span>
                  </template>
                </el-table-column>
                <el-table-column property="price" label="单价(元)" width="100" >
                  <template slot-scope="scope">
                    <el-input size="mini"  v-if="scope.row.status!=='在库'" v-model="scope.row.price" placeholder="请输入内容" ></el-input>
                    <span>{{scope.row.price}}</span>
                  </template>
                </el-table-column>
                <el-table-column property="unit" label="单位" width="100" >
                  <template slot-scope="scope">
                    <el-select size="mini"  v-if="scope.row.status!=='在库'" class="inputwidth" v-model="scope.row.unit" placeholder="请选择" >
                ...
                <el-table-column property="num" label="数量" width="80">
                  <template slot-scope="scope">
                    <el-input size="mini" v-if="scope.row.status!=='在库'"  v-model="scope.row.num" placeholder="请输入内容" ></el-input>
                ...
                <el-table-column property="warehousename" label="所在仓库" width="200">
                  <template slot-scope="scope">
                    <el-autocomplete class="autoInputwidth" v-model="scope.row.warehousename" v-if="scope.row.status!=='在库'||scope.row.status==undefined" :fetch-suggestions="querySaleContractWarehouseSearchAsync" placeholder="请输入仓库名称">
                      <template slot-scope="{ item }">
                        <div class="name">{{ item.value }}</div>
                        <span hidden>{{ item.id }}</span>
                        <el-tooltip  content="删除后重新点击输入框刷新" placement="bottom" effect="light">
                ...
                <el-table-column property="stockouttype" label="出库方式" width="150">
                  <template slot-scope="scope">
                    <el-select size="mini"  v-if="scope.row.status!=='在库'" class="inputwidth" v-model="scope.row.stockouttype" placeholder="请选择" >
                ...
                <el-table-column property="quality" label="品级" width="150" >
                  <template slot-scope="scope">
                    <el-select size="mini" v-if="scope.row.status!=='在库'" class="inputwidth" v-model="scope.row.quality" placeholder="请选择" >
                ...
                <el-table-column fixed="right" label="操作" width="120">
                  <template slot-scope="scope" v-if="scope.row.status!=='在库'">
                    <el-button @click.native.prevent="deleteRow(scope.$index, productGridData)" type="text" size="small">
                      移除
                    </el-button>
    ...
    <el-col :span="2">
      <el-dialog title="供应商管理" :visible.sync="supplyerVisible">
        <div>
          ...
          <el-table style="width:1000px" :data="supplyerGridData" max-height="500" :span-method="arraySpanMethod" class="el-tb-edit" ref="supplyerGridData" highlight-current-row>
            ...
            <el-table-column fixed="right" label="操作" width="120">
              <template slot-scope="scope">
                <el-button @click.native.prevent="deleteSupplyerRow(scope.row,scope.$index, supplyerGridData)" type="text" size="small">
                  移除
                </el-button>
                <el-button @click.native.prevent="saveSupplyerRow(scope.row)" type="text" size="small">
                  保存
                </el-button>
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {
    ...
    saveSupplyerRow(row) {
      ...
      this.axios
        .post(process.env.API_ROOT + "/PurchaseContractApi/v1/saveSupplyer", params)
        .then(response => {
          if (!response.data) {
    ...
    delSupplyerRow(row) {
      ...
      params.append("id", row.id);
      this.axios
        .post(process.env.API_ROOT + "/PurchaseContractApi/v1/delSupplyerRow", params)
        .then(response => {
          if (!response.data) {
            _this.listLoading = false;
            return;
    ...
    copyContract(row) {
      ...
      this.axios
        .post(process.env.API_ROOT + "/PurchaseContractApi/v1/copyContract", params)
        .then(response => {
          if (!response.data) {
            _this.listLoading = false;
            return;
    ...
    supplyerShow() {
      ...
      params.append("memberId", _this.memberId);
      this.axios
        .post(process.env.API_ROOT + "/PurchaseContractApi/v1/findSupplyerByPage", params)
        .then(response => {
          if (!response.data) {
            _this.listLoading = false;
            return;
   ...
   //读取合同仓库
    findSaleContractWarehouse() {
      ...
      params.append("memberId", this.memberId);
            ...
            this.axios
              .post(
                process.env.API_ROOT + "/SaleContractApi/v1/findSaleContractWarehouse",
                params
              )
              .then(res => {
                if (res.data.status === 200) {
                  let saleContractWarehouseListData = res.data.data.saleContractWarehouseList;
    ...
    async addContract() {
      ...
      params.append("purchaseContractDetail", JSON.stringify(_this.productGridData));
      if (_this.productGridData.length === 0) {
        this.message(true, "商品明细不能为空！", "error");
        return;
      }
      this.axios
        .post(process.env.API_ROOT + "/PurchaseContractApi/v1/addConstract", params)
        .then(response => {
          if (!response.data) {
            return;
          }
    ...
    verify(row) {
      ...
      this.axios
        .post(
          process.env.API_ROOT + "/PurchaseContractApi/v1/verifyPurchaseContract",
          params
        )
        .then(response => {
          if (!response.data) {
