...
export default {
  ...
  methods: {
    ...
    async getFormData() {
      ...
      let data = this.axios
        .post(
          process.env.API_ROOT + "/PermisApi/v1/findPermissionByPage",
          params
        )
        .then(response => {
          if (!response.data) {
    ...
    addPermis() {
      ...
      params.append("url", _this.ruleForm.url);
      params.append("type", _this.ruleForm.type);
      params.append("lastId", _this.ruleForm.baseType);
      this.axios
        .post(process.env.API_ROOT + "/PermisApi/v1/addPermissions", params)
        .then(response => {
          if (!response.data) {
            return;
          }
    ...
    delPermis() {
      ...
      this.axios
        .post(process.env.API_ROOT + "/PermisApi/v1/delPermis", params)
        .then(response => {
          if (!response.data) {
            return;
          }
    ...
    getCboData() {
      let _this = this;
      let params = new FormData();
      params.append("id", "d73d7a9c471f4aba884e79152cb1ace0");
      this.axios
        .post(
          process.env.API_ROOT + "/SysApi/v1/findAttributeDetailByAttrId",
          params
        )
        .then(response => {
          if (!response.data) {
    ...
    getBaseCboData(val) {
      ...
      this.axios
        .post(
          process.env.API_ROOT + "/PermisApi/v1/findLastPermissionByType",
          params
        )
        .then(response => {
          if (!response.data) {
          ...
          if (response.data.status === 200) {
            _this.baseTypes = response.data.data;
          } else {
            _this.message(true, response.data.msg, "error");
            _this.baseTypes = [];
          }
