...
<template>
	<el-row>
		...
		<el-col>
			<el-table v-loading="listLoading" element-loading-text="拼命加载中" :data="tableData1" @selection-change="handleSelectionChange1">
		...
		<el-col>
			<div class="block" style="float: right;margin-right: 10px;margin-top: 10px;">
				<el-pagination @size-change="handleSizeChange1" @current-change="handleCurrentChange1" :current-page="startPage1" :page-sizes="pageSizes1" :page-size="pageSize1" layout="total, sizes, prev, pager, next, jumper" :total="total1">
				</el-pagination>
			</div>
		</el-col>
		...
		<el-dialog title="属性明细" :visible.sync="dialogTableVisible1">
	     ...
	     { required: true, message: '请输入属性明细名称', trigger: 'blur' }
			...
			<div class="block" style="float: right;margin-right: 10px">
				<el-pagination @size-change="handleSizeChange2" @current-change="handleCurrentChange2" :current-page="startPage2" :page-sizes="pageSizes2" :page-size="pageSize2" layout="total, sizes, prev, pager, next, jumper" :total="total2">
				</el-pagination>
			</div>
		</el-dialog>
	</el-row>
...
export default {
  ...
  methods: {
    ...
    async getFormData1() {
      ...
      this.axios
        .get(process.env.API_ROOT + "/SysApi/v1/findAttributesByPage", params)
        .then(response => {
          if (!response.data) {
    ...
    async addAttr() {
      ...
      this.axios
        .post(process.env.API_ROOT + "/SysApi/v1/addAttributes", params)
        .then(response => {
          if (!response.data) {
            return;
          }
    ...
    async delAttrs() {
      ...
      this.axios
        .post(process.env.API_ROOT + "/SysApi/v1/delAttributes", params)
        .then(response => {
          if (!response.data) {
            return;
          }
    ...
    async addDetail() {
      ...
      this.axios
        .post(process.env.API_ROOT + "/SysApi/v1/addAttributeDetail", params)
        .then(response => {
          if (!response.data) {
            return;
          }
    ...
    async selDetails(id) {
      ...
      this.axios
        .get(
          process.env.API_ROOT + "/SysApi/v1/findAttributesDetailByPage",
          params
        )
        .then(response => {
          if (!response.data) {
    ...
    async delDetails() {
      ...
      this.axios
        .post(process.env.API_ROOT + "/SysApi/v1/delAttributeDetails", params)
        .then(response => {
          if (!response.data) {
            return;
          }
